...
class Lung3D_eccv_patient_supcon(Data.Dataset):
    def __init__(self, train=False, val=False, inference=False, n_classes = 2, supcon=False, box_lung=False, seg_sth=None, iccv_test=False, add_mosmed=False):
        ...
        if self.train:
            self.transform = tio.Compose([
                ...
                tio.RandomNoise(std=0.01, p=0.25),
        ...
        self.root_dir = "/remote-home/kejinlu/CVPR2026/challenge2/3d-norm/"
        ...
        if inference:
            ...
            for scan in sorted(os.listdir(test_path), key=natural_key):
                if scan.endswith(".npy"):
                    img = os.path.join(test_path, scan)
                    ID = scan
                    self.datalist.append((img, -1, ID))   # label=-1
            print("test samples:", len(self.datalist))
            return
        ...
        for mode in modes:
            for i, t, in enumerate(types):
                for j, s in enumerate(subfolders):
                    path = os.path.join(self.root_dir, mode, t, s)
                    for scan in sorted(os.listdir(path), key=natural_key):
                        img = os.path.join(path, scan)
                        name = t+'/'+scan
                        if box_lung:
                            self.datalist.append((img, j, name, self.train_dict[name.split('.')[0]]))
                        else:
                                self.datalist.append((img, j, name)) 
...
class Lung3D_ccii_patient_clf(Data.Dataset):
    def __init__(self, train=False, val=False, inference=False, n_classes = 2):
        ...
        self.root_dir = "/remote-home/share/21-yuanruntian-21210240410/cvpr24/challenge1/3d-norm/"
        types = ['non-covid','covid']
        ...
        if val:
            for i in range(len(types)):
                for scan in sorted(os.listdir(self.root_dir+'valid/'+types[i]+'/')):
                    img = os.path.join(self.root_dir+'valid/'+types[i]+'/', scan)
                    name = types[i]+'/'+scan
                    self.datalist.append((img, i, name))
        if inference:
...
def augment(sample, ifhalfcrop=True, ifrandom_resized_crop=True, ifflip=False, 
            ifrotate=True, ifcontrast=True,ifswap = False,filling_value=0):
    if ifhalfcrop:
