...
from models.resnet3D import resnet50, resnet34 #medical
...
class ResNet503D(nn.Module):
    ...
    def forward(self, x):
        ...
        return feat_map, features, logits
...
model_dict = {
    ...
    'medicalnet_resnet50': [resnet50, 2048],
    'medicalnet_resnet34': [resnet34, 512],
    'resnest50_3D': [resnest50_3D, 2048],
    'P3DCResNet50': [P3DCResNet50, 2048]

}

class SupConResNet(nn.Module):
    """backbone + projection head"""
    def __init__(self, name='c2dresnet50', ipt_dim=1, head='mlp', feat_dim=128, n_classes=2, supcon=False):
        super(SupConResNet, self).__init__()
        model_fun, dim_in = model_dict[name]
        self.encoder = model_fun(num_classes=n_classes, ipt_dim=ipt_dim)
    ...
    def forward(self, x):
        feat_map,feat, x = self.encoder(x)
        if self.supcon:
            feat = F.normalize(self.head(feat), dim=1)
        return feat_map,feat,x 
