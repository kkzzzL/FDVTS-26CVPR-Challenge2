            ...
            if train:
                ...
                if iccv_test:
                    ...
                    for label in range(len(types)):
                        path = '/home/feng/hjl/eccv-submit/test/final_tta/'+types[label]+'_p.csv'
                        ...
                        for i in range(select_num):
                            self.datalist.append(('./data/eccv/3d-data-norm/iccvtest/'+img_list[i][0]+'.npy', label, img_list[i][0]))

            if val:
                for i in range(len(types)):
                    for scan in sorted(os.listdir(self.root_dir_val+'valid/'+types[i]+'/')):
                        img = os.path.join(self.root_dir_val+'valid/'+types[i]+'/', scan)
                        name = types[i]+'/'+scan
                        if box_lung:
